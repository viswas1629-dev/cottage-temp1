    ...
    >
      <div className="max-w-9xl mx-auto px-4 sm:px-6 lg:px-10">
        <div className="flex items-center justify-between">
          ...
          >
            <div className={`p-1.5 sm:p-2 rounded-full transition-transform duration-300 group-hover:scale-105 flex-shrink-0 ${isScrolled || (!isHomePage && !mobileMenuOpen)
            ...
            <div className="flex flex-col min-w-0 justify-center">
              ...
              <div className="flex sm:hidden items-start gap-2 leading-none">
                <div
                  className={`font-mermaid text-[25px] lg:text-base font-bold flex flex-col leading-tight tracking-tight min-w-0 ${isScrolled || (!isHomePage && !mobileMenuOpen)
                      ? 'text-[#6caca0]'
                ...
                <div
                  className={`font-serif italic text-[24px] lg:text-[16px] font-bold flex-shrink-0 mt-8.5 -translate-x-3 ${isScrolled || (!isHomePage && !mobileMenuOpen)
                      ? 'text-[#6caca0]'
                ...
                <div
                  className={`font-serif italic text-[24px] lg:text-[16px] font-bold flex-shrink-0 mt-8.5 -translate-x-3.5 ${isScrolled || (!isHomePage && !mobileMenuOpen)
                      ? 'text-[#c075b7]'
              ...
              <div className="hidden sm:flex items-baseline gap-1.5 lg:gap-2 leading-none">
                <span
                  className={`font-mermaid text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight ${isScrolled || (!isHomePage && !mobileMenuOpen)
                      ? 'text-[#6caca0]'
                      : 'text-[#6caca0] drop-shadow-[0_1px_2px_rgba(0,0,0,0.45)]'
                    }`}
                >
                ...
                <span
                  className={`font-mermaid italic text-2xl sm:text-3xl lg:text-4xl font-bold flex-shrink-0 ${isScrolled || (!isHomePage && !mobileMenuOpen)
                      ? 'text-[#c075b7]'
                      : 'text-[#c075b7] drop-shadow-[0_1px_2px_rgba(0,0,0,0.35)]'
                    }`}
                >
              ...
              {/* Subtitle */}
              <span
                className={`text-[8px] xs:text-[9px] sm:text-[9.5px] tracking-[0.18em] uppercase font-sans mt-1 font-medium truncate ${isScrolled || (!isHomePage && !mobileMenuOpen)
                    ? 'text-[#FAF7F2]'
                    : 'text-white/85'
                  }`}
              >
          ...
          <div className="hidden md:flex items-center gap-3">
            <a
              ...
              className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-semibold tracking-wide transition-all duration-300 shadow-sm hover:shadow-md cursor-pointer ${isScrolled || !isHomePage
          ...
          <div className="flex md:hidden items-center gap-1.5 sm:gap-2 flex-shrink-0">
            <a
              href="tel:+919344950030"
              aria-label="Call +91 93449 50030"
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all shadow-sm ${isScrolled || (!isHomePage && !mobileMenuOpen)
            ...
            <button
              ...
              className={`p-1.5 sm:p-2 rounded-lg transition-colors focus:outline-none ${isScrolled || (!isHomePage && !mobileMenuOpen)
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`text-2xl font-serif tracking-wide py-1 flex items-center justify-between transition-colors ${isActive ? 'text-[#C89D66] font-medium pl-2 border-l-2 border-[#C89D66]' : 'text-white/80 hover:text-white'
                      }`}
                  >
                    <span>{link.name}</span>
